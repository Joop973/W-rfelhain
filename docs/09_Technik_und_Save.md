...
- **Sync 2026-07-11:** dazugekommen: `fluch.js` (07 §5.4) · **SAVE_VERSION 7** (v6: hinweise/wendungGesehen/hpMax; v7: `kampfKnoten` {knotenId, seed} — Reload mid-Kampf startet DENSELBEN Kampf von vorn, Anti-Scumming + Tab-Rauswurf-Schutz) · `uebersetze(key, params)` mit `{name}`-Platzhaltern, UI-Chrome vollständig auf Keys (~120 `ui.*`/`status.*`) · Sprite-Manifest-Ladeebene (`assets/manifest.json`, Fallback Platzhalter-Box) · Arena-Render nach Artefakt 12 · Werkzeuge `tools/sprite_freistellen.py` + `tools/geraete_smoke.mjs` · **189 Tests grün**.
