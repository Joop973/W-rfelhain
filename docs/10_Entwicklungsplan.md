...
| C — Meta | ✅ komplett | — |
| D — Voller Umfang (Tor 3) | ✅ bis auf D6 | **D6**: Kunst/Audio (87 Assets da: **alle Böden/Kulissen R1–R6** + HUD komplett + R1-Figuren; offen: Gegner R2–R6 (dein Bildmodell), VFX, Audio) |
...
- [ ] **E7 [C] Restposten 2** *(klein, nichts davon blockiert E3/E4)*:
  ...
  - [x] **HUD auf Holz-Plaketten umgebaut** ([D]-Entscheid 2026-07-11 „auf Plaketten umbauen"): Atem-Pips (`atem.pip`/`_leer`-Blattspiralen statt Kreis-Orb); Kampf-Aktionsknöpfe als geschnitzte Plaketten (`knopf.reroll` = Neu werfen, `knopf.wurf` = Auflösen/Gegnerzug/Weiter, `.gedrueckt`-Frame auf :active, heller Text mit Schatten); Panels (`ph--belohnung`: Belohnung/Schmiede/Markt/Event/Lagerfeuer) mit `rahmen.9slice` als border-image + Pergament-Füllung (fill), Fallback auf Farbe wo Bild fehlt. Screenshots verifiziert (Kampf + Sieg-Panel), Touch-Ziele 50 px, 189 Tests + Smoke grün
  - [ ] optional später: Übermut-Leiste-Sprite (`uebermut.leiste`) hat feste Füllung → nicht dynamisierbar, CSS-Pips bleiben; `overlay.gesperrt`/`schreck.marke` (kein Seiten-Gitter in der Armee-Ansicht) — geringer Gewinn, zurückgestellt
